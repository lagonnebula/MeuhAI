...
AS_LIF_MENT.SkillID = 8004
AS_LIF_MENT.HowLast = (600 - (AS_LIF_MENT.Level * 300)) * 1000
AS_LIF_MENT.Engaged = false
AS_LIF_MENT.TimeOut = 0
...
--------------------------------------------------
	...
	or HomunType == VANILMIRTH2 or HomunType == VANILMIRTH_H2) then
		if GetV(V_HP, OwnerID)/GetV(V_MAXHP, OwnerID) < 1 then
			DoSkill(AS_VAN_BLES, OwnerID)
		end
	end

	if(HomunType == LIF  or HomunType == LIF_H
	or 	 HomunType == LIF2 or HomunType == LIF_H2)
	then
		if GetV(V_HP, OwnerID)/GetV(V_MAXHP, OwnerID) < 0.8 then
			DoSkill(AS_LIF_HEAL, OwnerID)
		end
	end

	-- MEUH if (ANTI_AFK_MODE ~= true) then
	if CurrTime - StartupTime > 1100 then
		if(HomunType == LIF  or HomunType == LIF_H
		or     HomunType == LIF2 or HomunType == LIF_H2
	...
	if CurrTime - StartupTime > 300 then
...
--------------------------------------------------
	...
	or HomunType == LIF2 or HomunType == LIF_H2
	or HomunS == true and OldHomunType == LIF) then
		DoSkill(AS_LIF_ESCP, MyID) -- MEUH :: Lif: Urgent Escape
	...
	if (MySkill == 0) then
		...
		if (EnemyType < 1078 or EnemyType > 1085) then -- don't waste SP on plants and mushrooms
			...
			--------------------------------------------
				...
				-- MEUH :: Add Mental Change skill
				if CanDoAtkSkillsNow() then
					if DoSkill(AS_LIF_MENT, MyID) then
						AtkSkillDoneCount = AtkSkillDoneCount + 1
					end
				end
...
--------------------------------------------------
	-- MEUH :: Je peut faire un truc potentiellement
end
